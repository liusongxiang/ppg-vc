...
class MelDecoderLSA(AbsMelDecoder):
    def __init__(
        ...
        if self.multi_speaker:
            ...
            self.reduce_proj = torch.nn.Linear(bottle_neck_feature_dim + spk_embed_dim,
                                               bottle_neck_feature_dim)
    ...
    def forward(
        ...
        if self.use_bnf_prenet:
            ...
            if self.use_instance_norm:
                self.norm_layer(decoder_inputs.transpose(1,2)).transpose(1,2)
        if self.use_pitch_info:
